/**
 * Sync diagnostics card for the Ops editor — the dashboard counterpart of the
...
export function SyncDebugSection({ data, flash }: any) {
  ...
  return h('section', { className: 'card card-debug' },
    ...
    h('div', { className: 'card-body' },
      ...
      links.length === 0
        ? h('p', { className: 'empty-line muted' }, 'No live links on this server yet. Create one above, then come back here to inspect it.')
        : h(React.Fragment, null,
            h('div', { className: 'debug-toolbar' },
              h('label', { className: 'field debug-link-pick' },
                h('span', { className: 'field-label' }, 'Link'),
                h('select', { className: 'input', value: linkId, onChange: (e: any) => setLinkId(e.target.value) },
                  links.map((item) => h('option', { key: item.id, value: item.id }, linkLabel(item))))),
...
function ScanTool({ link, names, flash, onDone }: any) {
  ...
  async function run(apply: boolean) {
    if (apply) {
      ...
      if (!confirm(`Repair ${count} issue(s)? This posts missing messages, edits stale copies and deletes leftover or filtered copies in the linked channels.`)) return;
    }
  ...
  const counts: Record<string, number> = { missing: 0, stale: 0, 'copy-missing': 0, orphan: 0, filtered: 0 };
  ...
  return h('div', { className: 'debug-panel' },
    ...
    h('div', { className: 'debug-controls' },
      h('label', { className: 'field' }, h('span', { className: 'field-label' }, 'Messages to compare'),
        h('select', { className: 'input', value: limit, onChange: (e: any) => setLimit(e.target.value) },
          ['50', '200', '500', '1000', '2000'].map((n) => h('option', { key: n, value: n }, `Newest ${n}`)))),
      link.mode === 'twoway'
...
function LedgerTool({ link, names, flash }: any) {
  ...
  return h('div', { className: 'debug-panel' },
    h('div', { className: 'debug-controls' },
      ...
      h('select', { className: 'input input-narrow', value: limit, onChange: (e: any) => { setLimit(e.target.value); load(e.target.value); } },
        ['25', '50', '100', '200'].map((n) => h('option', { key: n, value: n }, `Newest ${n}`))),
...
function ProbeTool({ link, names, flash }: any) {
  ...
  async function run() {
    if (!confirm(`Post a test message in ${probeName}, edit it, then delete it?`)) return;
    setBusy(true);
    try {
...
function MaintenanceTool({ link, flash, onDone }: any) {
  ...
  async function replay() {
    ...
    if (!confirm('Mirror this message again? Its old copy is deleted and a new one is posted at the bottom of the channel.')) return;
    setBusy('replay');
    try {
  ...
  async function runPrune(apply: boolean) {
    if (apply && !confirm(`Remove ${prune ? prune.deadLinks + prune.missingCopies : 'the dead'} ledger row(s)? Only records are removed — no messages are touched.`)) return;
